...
##        f.write('\n')
    ...
    for i in range(nums):
        ...
        with open(filename, 'a') as f:
             ...
             f.write(',')
             f.write(str(position[0]*1000))
             f.write(',')
             f.write(str(position[1]*1000))
             f.write(',')
             f.write(str(position[2]*1000))
             f.write(',')
             f.write('\n')
...
##    # 在body中添加position
##    doc.append(position_list)

    # 使用JSON写CZML文件

##  json.dump(position_list, f)

# 使用json写轨道信息JSON文件
##with open('orbit_info.json', 'w') as f:
